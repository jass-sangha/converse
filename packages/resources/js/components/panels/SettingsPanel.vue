...
const filteredRows = computed(() => {
    ...
    if (!q) return ROWS;
    return ROWS.filter((row) => row.label.toLowerCase().includes(q) || row.hint.toLowerCase().includes(q));
});

const sectionTitle = computed(() => ROWS.find((row) => row.key === section.value)?.label ?? '');
    ...
    <div class="cv-settings-panel flex h-full flex-col bg-converse-surface">
        <template v-if="!section">
            ...
            <div class="cv-settings-panel__avatar flex flex-col items-center gap-3 pb-4">
                <span class="rounded-full bg-converse-surfaceHover px-3 py-1 text-xs text-converse-textMuted">Available</span>
                <label class="cv-settings-panel__avatar-upload group relative cursor-pointer rounded-full">
                    <Avatar :name="me?.name ?? ''" :avatar-url="me?.avatar_url" :size="96" />
            ...
            <div class="cv-settings-panel__rows flex-1 overflow-y-auto border-t border-converse-border">
                <button
                ...
                >
                    <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" class="shrink-0 text-converse-textMuted"><path :d="row.path"/></svg>
                    <span class="min-w-0">
                        <span class="block text-[15px] text-converse-text">{{ row.label }}</span>
                        <span class="block truncate text-xs text-converse-textMuted">{{ row.hint }}</span>
                    </span>
                </button>
                ...
                <button
                    ...
                    @click="logout"
                >
                    <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" class="shrink-0"><path d="M10 3v2H5v14h5v2H3V3h7Zm5.29 3.71L18.59 10H8v2h10.59l-3.3 3.29 1.42 1.42L22 11.41l-5.29-5.3-1.42 1.6Z"/></svg>
        ...
        <template v-else>
            <div class="cv-settings-panel__section-header flex items-center gap-3 border-b border-converse-border px-3 py-3">
                <button type="button" class="flex h-9 w-9 items-center justify-center rounded-full text-converse-textMuted hover:bg-converse-surfaceHover" @click="section = null">
                ...
                <h2 class="text-lg font-semibold text-converse-text">{{ sectionTitle }}</h2>
            </div>

            <div class="cv-settings-panel__section-body flex-1 overflow-y-auto p-4">
                <template v-if="section === 'profile'">
                    <div class="mb-6 flex flex-col items-center gap-2">
                        <label class="group relative cursor-pointer rounded-full">
                            <Avatar :name="me?.name ?? ''" :avatar-url="me?.avatar_url" :size="120" />
                            <span class="absolute inset-0 flex items-center justify-center rounded-full text-xs font-medium text-white opacity-0 transition group-hover:bg-converse-overlay/40 group-hover:opacity-100">
                                {{ uploading ? 'Uploading…' : 'Change photo' }}
                            </span>
                            <input type="file" accept="image/*" class="hidden" :disabled="uploading" @change="onFileChange">
                        </label>
                ...
                <template v-else-if="section === 'privacy'">
                    <div class="mb-3 rounded-cv border border-converse-border">
                        <div class="flex items-center justify-between border-b border-converse-border p-3">
                    ...
                    <button type="button" class="w-full rounded-cv border border-converse-border p-3 text-left text-sm text-converse-text hover:bg-converse-surfaceHover" @click="showBlocked = true">
                        Blocked contacts
                    </button>
                </template>
                ...
                <template v-else>
                    <p class="text-sm text-converse-textMuted">This setting isn't available yet.</p>
                </template>
            </div>
        </template>
