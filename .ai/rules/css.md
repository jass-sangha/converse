...
Why: without the layer wrapper, the reset's `#riwaaq-chat-app [type='button'] { background-color: transparent }` rule has the same specificity as any `.bg-riwaaq-*` utility class on a `<button>`. Since it's plain CSS placed after `@tailwind utilities;` in the source, it compiles later in the stylesheet and silently wins every tie — blanking the background of every button that uses a `bg-riwaaq-*` class (switch tracks, the sidebar "+ New" button, etc. were all invisible from this before it was fixed).
...
The hand-ported Preflight subset in app.css (see [[Custom preflight-port rules must stay inside @layer base]]) must include `#riwaaq-chat-app a { color: inherit; text-decoration: inherit; }`.
...
If you touch this reset block again, keep both the button and anchor resets — they're the two element types most likely to carry `bg-riwaaq-*`/`stroke="currentColor"` utilities without an explicit color class.
